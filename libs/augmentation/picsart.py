...
class PicsartData(torch.utils.data.Dataset):
    def __init__(self, data_root, source_list, tokenizer, style_name, use_text_inversion, augmentation, image_size, clip_similars=None):
        ...
        self.similar_captions = torch.load(clip_similars, map_location='cpu', weights_only=True)
        if isinstance(self.similar_captions, torch.Tensor):
            self.similar_captions = self.similar_captions.tolist()
    ...
    #         print("Warning: Expected a string, but got a dictionary. Attempting to extract 'caption'.")
    #         raw = raw.get("caption", "")  # Extract caption or return empty string
    #     if isinstance(raw, str):
    #         word_list = raw.split()
    #     else:
    #         raise ValueError("Expected a string for caption, but got {}".format(type(raw)))
    #     # Continue with synonym replacement logic here
    #     return raw
    ...
    def __getitem__(self, idx):
        ...
        raw_caption = self.data[file_name]['caption']  
        if len(self.augmentation) > 0:             
            if 'retrieval' in self.augmentation and file_name in self.similar_captions:
                ...
                caption_str = self.data[file_name]['caption']  
