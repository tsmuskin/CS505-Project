            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if config['use_content_loss']:
                    if step != 0:
                        ...
                        # if fake_image.mode in ["P", "LA"]: # Check if the image is in P (palette-based) mode
                        fake_image = fake_image.convert("RGBA") 
                        ...
                        gt_image = Image.open(batch['image_path'][0])
                        # Check if the image is in 'P' mode and convert to 'RGBA' if necessary
                        # if gt_image.mode in ["P", "LA"]:
                        gt_image = gt_image.convert("RGBA")
                        gt_input = image_processor(images=gt_image, return_tensors='pt')['pixel_values'].to(accelerator.device)
